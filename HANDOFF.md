...
>
> **Still honestly incomplete, by design**: ME's Personalization status (§14.4, "what LifeOS currently uses vs tentative vs disabled" as one explained summary) and Data sources; Weekly Reset's Adjustment (§13.5) — a distinct concept (proposed *operating changes* for the week ahead, not just a preference confirmation) needing its own design, not just reusing the Insight loop.
>
