...
> **Status addendum 2026-09-15 (latest).** `main` now includes the full priority order from this session's module inventory: Epic #8 design system V1 (PR #51) → B5 + W5 + quick capture + Inbox/Incubator + Get Unstuck V0 (PR #53, supersedes stacked PR #52 — see "Rebase note" below) → Execute landing V0 (PR #54, §7.1) → ME overview V0 (PR #55, §14.1, personal context only) → Direction edit (PR #56, §6.1, title/description) → **Weekly Reset V0 (PR #57, §13, Reality/Movement/Next Week from recorded facts; Pattern Candidates/Adjustment named as not yet available — same "no fabricated confidence" rule as ME)**. Current `main` head: `b220992718e731de08fac363b28d01b8c8e6b213`. Every PR from #53 onward was verified against a real local Postgres before merging, and each was built as its own branch off the then-current `main` tip + squash-merged individually (no more stacking, avoids the squash-merge rebase problem hit with #52).
>
> **PR #58 (2026-09-15, latest)** closed most of that gap: implemented the canonical `Insight` and `OperatingPreference` entities (`DOMAIN_MODEL_V1.md` §16-17, both P0 in `MVP_SCOPE_V1.md` #14/#17). Two deterministic threshold detectors (no AI/ML) — `duration_completion_rate` and `project_load_stall_rate` — persist candidates on read (migration `0010`, tables `insights` + `operating_preferences`), confirmed via Confirm/Partly-accurate/Reject (Incorrect+Do-not-use merged into one `rejected` status, product owner decision). Confirmed preferences are wired into the Next Action Engine's ranking (`duration_preference`/`project_load_preference` factors, only appear when a matching preference is active). ME now shows live Operating Preferences (edit/delete/disable) and Pattern Candidates (confirm/edit/reject); Weekly Reset surfaces up to 3 fresh Pattern Candidates (spec §13.4 "Max 3").
>
> **PR #59** closed §13.5 Adjustment: re-read Meeting #010 §12 Step 4 (its own example, "Default generated Next Actions to ≤45 min", is exactly an OperatingPreference) and concluded Adjustment is the same Insight-confirmation action from §13.4/§14.3, not a second entity or a second confirmation step — shipped as an `effect` field on `ProposedPreference` (static per-pattern template, never AI-written) rendered as "Nếu xác nhận: …" on the same Pattern Candidate cards in both ME and Weekly Reset. The spec-literal two-step reading (validate the pattern, then separately accept the resulting change) was considered and explicitly deferred rather than reworking PR #58's already-shipped `resolveInsight` semantics for a mostly-narrative distinction — flag this if the product owner later wants stricter 5-step fidelity.
>
> **Still honestly incomplete, by design**: ME's Personalization status (§14.4, "what LifeOS currently uses vs tentative vs disabled" as one explained summary) and Data sources.
>
> Also still open from the branch map below: cherry-picking Admin panel + first-run onboarding from `feat/ui-design-system-v1`, and closing out the other abandoned parallel branches.
>
