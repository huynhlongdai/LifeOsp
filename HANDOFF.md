...
>
> **What's now honestly incomplete, by design** (both surfaced to the user instead of faked): ME's Operating Preferences/Pattern Candidates/Personalization status (§14.2-14.4) and Weekly Reset's Pattern Candidates/Adjustment (§13.4-13.5) all need a preference/pattern-detection model with evidence-based confidence scoring that doesn't exist in this codebase yet. Building that is the natural next slice if the product owner wants it — it's a real AI/inference feature, not a V0 gap-filler.
>
